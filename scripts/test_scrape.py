from scrape import parse_tickers_from_index, parse_dividend_page
...
def test_parse_dividend_page_parses_fact_row():
    ...
    assert lkoh_2025['isForecast'] is False
...
def test_parse_dividend_page_parses_forecast_row():
    ...
    assert f['isForecast'] is True


def test_parse_dividend_page_handles_img_tags_in_cells():
    result = parse_dividend_page(DIVIDEND_HTML, 'LKOH')
    # Прогнозная строка содержит <img> теги — должны быть проигнорированы
...
from scrape import fetch_with_retry
